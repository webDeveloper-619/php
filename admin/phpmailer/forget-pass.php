...
$reg=str_replace("www.", "", $_SERVER['HTTP_HOST']);
$reg=str_replace(".net", "", $reg);
$reg=str_replace(".com", "", $reg);
			...
			<table>
				<tr>
					<td>
						...
						<h3>Hello,".$row['user_name']." ".$row['last_name']."</h3>
						<p>You just changed your password</p>
						<p>If you didn’t change your password, give us a call right away at +1-631-440-3811.</p>
						<p>Your new password is : ".$row['password']."</p>
						<p>Just a reminder:</p>
						<ul>
...
$mail->setFrom('contact@tvcnw.com', $host);

$mail->addAddress($row['email'], $row['user_name']);

if ($mail->addReplyTo('contact@tvcnw.com')) {
    $mail->Subject = 'Do not reply this email';
    //$mail->WordWrap = 50000;               // set word wrap
    $mail->Priority = 1; 
    $mail->IsHTML(true); 
    $mail->Body    = $message;
    $mail->AltBody = $message; //'your password is: '.$password;
        if (!$mail->send()) {
